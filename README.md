...
Open <http://127.0.0.1:8090>. The app discovers llama.cpp models, ComfyUI checkpoints, and saved workflows. Open **Advanced models & workflows** to override its selections.
...
No model names are hard-coded. By default the app reads llama.cpp's `/v1/models` (including router presets) and ComfyUI's `/models/checkpoints`. A choice in the Advanced panel applies to that film; `LLAMA_MODEL` and `COMFY_CHECKPOINT` set machine-wide defaults.
...
The included [`workflows/image.json`](workflows/image.json) uses standard ComfyUI nodes and whichever installed checkpoint is selected. Replace `COMFY_IMAGE_WORKFLOW` if the selected model requires a different architecture.

### Saved video workflows

The app lists workflows from ComfyUI's `userdata` API and recognizes image-to-video and text-to-video graphs. ComfyUI's normal **Save** command stores an editable UI graph; its remote prompt endpoint requires a flattened API graph. To make a saved video workflow executable:

1. Enable **Dev Mode** in ComfyUI settings.
2. Open the tested video workflow.
3. Choose **Save (API Format)** and place the resulting JSON in ComfyUI's saved workflows, or point `COMFY_VIDEO_WORKFLOW` at it.
4. In Local Movie Maker, open **Advanced models & workflows**, refresh, and select it.
...
- `POST /api/projects` — accepts `{"prompt":"...","duration":30,"resolution":"720p"}` plus optional `llama_model`, `checkpoint`, and `video_workflow` selections.
